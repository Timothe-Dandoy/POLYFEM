...
do    
    ...
    # Output one fasta per introns, with the sequence of each individual
    seqtk subseq ${fasta_haplotype} ${region_intron} | seqtk seq -r - > ${intron_allind}           
done

#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# 4. Build the mRNA sequence by merging the fasta of all exons
echo
echo "step 4"
echo

# Make a file with the list of all individuals IDs
awk '{print $1}' ${region_file_ex1} | sort -u > ${individual_file}        
...
# Database creation
makeblastdb -in ${fasta_allind_mRNA} -dbtype nucl      

# blast all individual's sequences in the coding sequence 
blastn -db ${fasta_allind_mRNA} -query ${fasta_ref_coding} -out ${out_blast_coding} -outfmt "6 qseqid sseqid pident length qstart qend sstart send evalue bitscore"    

# Make the region file needed for the coding sequence: ID_ind CDS_start CDS_stop  
Rscript $Make_RegionFile_coding_R $out_blast_coding $region_coding

# Output one fasta with the coding sequence of each individual
seqtk subseq ${fasta_allind_mRNA} ${region_coding} > ${fasta_allind_coding}                

#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# 6. Protein Translation, from nucleotide sequences to amino-acid sequences
echo
echo "step 6"
echo
...
#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# 7. Protein comparison between phenotypes : S and RB
echo
echo "step 7"
echo
...
#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# 8. Aligne exon and intron sequences
echo
echo "step 8"
echo
...
#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# 9. Nucleotide comparison between phenotypes : S and RB
echo
echo "step 9"
echo
...
# Make the region file needed for the EDNRB sequence: ID_ind EDNRB_start EDNRB_stop
...
#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# 12. Build the nexus file with the EDNRB sequence, and the phenotype of each individual
echo
echo "step 12"
echo

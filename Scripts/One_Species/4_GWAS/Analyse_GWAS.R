...
threshold  = 0.05/(1.5e9)
threshold _log = -log10(threshold )

#-----------------------------------------------------------------
# Scripts
#-----------------------------------------------------------------

# Regroup the positions along each chromosome in a single genome
for (i in 1:18){
...
# Compute the Pvalue of each position with the LRT
...
p <- ggplot(
  ...
  geom_hline(yintercept = threshold _log, color = "red") +
...
# Found the diagnostic position above the threshold 
Asso_Sign = Asso[Asso$MinusLogPval > threshold _log,]
Pos_sign = initial_Pos[Asso$MinusLogPval > threshold _log]
Chr_sign = Asso$Num_chr[Asso$MinusLogPval > threshold _log]
MLP_sign = Asso$MinusLogPval[Asso$MinusLogPval > threshold _log]
...
# Build the histogramme of Pvalue distribution 
hist <- ggplot(Asso, aes(x = MinusLogPval)) + 
  geom_histogram(
    bins=20,
    fill = colour,
    colour = "black"
  ) +
  labs(title = species, x = expression(-log[10](pvalue)), y ="Count") +
  theme_classic() +
  theme(
    plot.title = element_text(size = 16),
    axis.title = element_text(size = 14),
...
ggsave(
   paste0(prefix_res,"hist_Pval.png"),
   plot = hist,
   width = 10,
   height = 5,
   dpi = 150   

...
# Get phenotypic parameters database
...
# Construct the database for the PCA
...
#get coordinate of individuals along dimension
dim1 = c()
count = 1
for (el in IDs){
  if (el %in% BD_clean$Sample_ID){
    ...
    count = count + 1 
  } else {dim1 = c(dim1,-999)}         #if not in the PCA, the individual get the value -999 => NA value in Angsd
}
...
# List of the population name for every individual
writeLines(as.character(pops),pop_file)

# List of phenotype value (dimension 1 coordiante) for every individual
writeLines(as.character(dim1),pheno_file)

# Table with individual name associated with the corresponding phenotype
write.table(
  bd_pheno,
  file = ind_pheno_file,
  sep = "\t",
